...
### ✨ 新增特性与交互增强 (Features & UI Enhancements)
- **「智能正文」模式下网页切换/跳转的气泡提示 (Bubble Tooltip)**：
  - **触发机制**：当 SidePanel 处于打开状态且处于「智能正文」（`currentMode === 'article'`）状态时，若用户在浏览器中切换标签页（`tabs.onActivated`）或当前网页发生跳转/刷新（`tabs.onUpdated`），底部刷新按钮上方会自动弹出精致微动画气泡，提示用户点击重新提取正文。
  - **交互与容错**：支持直接点击气泡或刷新按钮完成更新；切换回「选区内容」Tab、点击右上角 `×` 或 12 秒无操作后自动平滑隐去。
  - **多语言适配**：气泡提示文案接入 `src/utils/i18n.js`，无缝支持中、英、繁、日、俄 5 国语言动态切换。
- **SidePanel 顶部视觉优化**：
  - 将侧边栏顶部品牌占位 SVG 替换为官方高保真品牌图标 `public/icons/icon32.png`，提升视觉一致性与原生质感。
